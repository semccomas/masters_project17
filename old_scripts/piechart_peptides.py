...
ax1.axis('equal')
ax1.set_title('Jejunum run 1')

ax2.pie(sizes_jejM, startangle =90, colors = colors)
ax2.axis('equal')
ax2.set_title('Jejunum run 2')
...
lgd = plt.legend(labels, loc = 'center right', bbox_to_anchor = (1.8,1.2), prop = {'size': 12})

plt.tight_layout()
#plt.show()
plt.savefig('piechart_peptides.png', format = 'png', dpi = 700, bbox_extra_artists=(lgd,), bbox_inches='tight')
